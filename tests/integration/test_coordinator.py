...
class TestAptusCoordinator:
    ...
    async def test_it_should_fetch_doors_status_and_bookings_on_update(
        ...
        with (
            patch.object(
                doors, "list_doors", return_value=MOCK_DOORS
            ) as mock_list,
            patch.object(
                doors, "get_apartment_door_status", return_value=MOCK_DOOR_STATUS
            ) as mock_status,
            patch.object(
                laundry, "list_bookings", return_value=MOCK_BOOKINGS
            ) as mock_bookings,
        ):
    ...
    async def test_it_should_raise_update_failed_on_connection_error(
        ...
        with patch.object(
            doors, "list_doors", side_effect=AptusConnectionError("timeout")
        ):
            coordinator = AptusDataUpdateCoordinator(hass, mock_client)
            await coordinator.async_refresh()
    ...
    async def test_it_should_trigger_reauth_on_auth_error(
        ...
        with patch.object(
            doors, "list_doors", side_effect=AptusAuthError("expired")
        ):
            coordinator = AptusDataUpdateCoordinator(hass, mock_client)
            await coordinator.async_refresh()
